...
def FlattenZstack(z_stack, z_stack_shape, mask, subsample=True, method="default",**kwargs):
    ...
    # Initialize empty sub mask
    sub_mask = None

    # Check to see if subsampling
    if subsample:
        # default subsampling method
        if method == "default":
            # print update
            logging.info("implementing default subsampling...")
            # capture size of coordinates
            sz = len(coords)
            # check for below 50k pixels
            if sz < 50000:
                # set subsampling to none
                logging.info("subsampling set to default, but there are less than 50k pixels...")
            ...
            # check for 50k-100k
                ...
                sub_mask, sub_coords = SubsetCoordinates(
                    coords=coords, array_size=mask.shape, method="pseudo_random", n=0.55, grid_spacing=(2, 2)
                )
            ...
            # check for 50k-100k
                ...
                # Subset the coordinates using custom function
                sub_mask, sub_coords = SubsetCoordinates(
                    coords=coords, array_size=mask.shape, method="pseudo_random", n=0.15, grid_spacing=(3, 3)
                )
                # Create an array from the sparse scipy matrix
                sub_mask = sub_mask.toarray()
                # Use the mask to extract all the pixels
                ...
                # Subset the coordinates using custom function
                sub_mask, sub_coords = SubsetCoordinates(
                    coords=coords, array_size=mask.shape, method="grid", grid_spacing=(3, 3)
                )
        ...
        # otherwise use user input for subsampling
        else:
            # Subset the coordinates using custom function
            sub_mask, sub_coords = SubsetCoordinates(
                coords=coords, array_size=mask.shape, method=method, **kwargs
            )
    ...
    return z_stack, coords, sub_mask
